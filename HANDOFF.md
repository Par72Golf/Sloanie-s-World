...
| `mountain.ts` | The ice cream mountain: climbs every flight to the summit and the telescope, shoves her sideways off all 42 treads in four directions, runs the ledges, and does twelve wobbly climbs. Fails if she ever leaves the stair or ledge other than by the slide. Before the rails: 119 of 126 shoves fell. |
| `model-boxes.ts` | Every registered model's colliders still include every box its factory declares. The Sugar Rush buildings register rows written down in `sugar-model-boxes.ts`; `DUMP=<id>` prints a factory's rows to paste back in when the drawing changes. |
