...
| `solid.ts` | **Sugar Rush.** Can she walk into it? Builds every model with a do-nothing canvas, slices what is drawn at knee-to-chest height and measures how far each piece stands outside the colliders. Fails past 0.35m. `MEANT` lists what she is supposed to pass through; `WHERE=1` names the offending part. Found the lollipops, cakes, stands, fountain, gumball machine, bridge rail drift and fence rails from the first real play. |
| `mountain.ts` | The ice cream mountain: climbs every flight to the summit and the telescope, shoves her off all 42 treads in seven directions (out, in toward the drum, and along both ways), runs the ledges, does twelve wobbly climbs, and looks at the air over every tread for anything drawn standing in it. A fall is more than a metre through the air landing off the stair. The second real play found the slot between the second flight and its drum and the ice cream lobes over the treads, both of which the first version passed; with the old mountain restored it now fails on both. |
| `model-boxes.ts` | Every registered model's colliders still include every box its factory declares. The Sugar Rush buildings register rows written down in `sugar-model-boxes.ts`; after changing the mountain run `tools/regen-mountain.sh`, and for another model `DUMP=<id>` prints its rows to paste back in. |
